...
def _load_training_data(conn) -> list[dict]:
    ...
    area_km2 (static land area) is pulled from country_metadata so
    _build_feature_matrix can use log(area) as a size anchor.
    """
    with conn.cursor() as cur:
        cur.execute("""
            ...
            pivoted AS (
                ...
                GROUP BY lp.iso2, lp.population, cm.area_km2
            )
            SELECT * FROM pivoted
...
def _build_feature_matrix(rows: list[dict]) -> tuple[np.ndarray, list[str]]:
    ...
    Returns (X_raw, iso2s) where X_raw has shape (n, len(ALL_FEATURE_KEYS)).
    """
    ...
    return np.array(X_rows, dtype=float), iso2s
...
def _persist_model(conn, weights: dict, scaler_mean: list, scaler_scale: list, residuals: dict) -> int:
    """
    Insert a new model_weights row (including scaler params) and all
    per-country residuals. Returns model_id.
        ...
        cur.execute(
            ...
            INSERT INTO model_weights
                (intercept, telecom, electricity, building, mobility, internet,
                 ...
                 n_training, scaler_mean, scaler_scale)
            VALUES
...
def run_training() -> dict:
    ...
    # Build feature matrix with mean imputation
    X_raw, iso2s = _build_feature_matrix(filtered)
    y = np.array([math.log(float(r["population"])) for r in filtered])
    log.info("[trainer] X shape: %s  |  y range: [%.2f, %.2f]", X_raw.shape, y.min(), y.max())

    # Per-feature Ridge: each feature gets its own penalty.  Telecom
    # (r≈0.98) gets near-zero regularisation.  Building, mobility, and
    # internet signals are heavily regularised (α=10) to effectively
    # prune them — their coefficients shrink to ~0.  Electricity gets
    # mild shrinkage.  log_area_km2 stays essentially unregularised.
    PER_FEATURE_ALPHAS = np.array([0.001, 0.1, 10.0, 10.0, 10.0, 1e-6])
    cv = KFold(n_splits=5, shuffle=True, random_state=42)

    # --- CV loop: compute out-of-fold R² with fixed per-feature alphas ---
    fold_mses = []
    fold_r2s = []
    ...
    # --- Fit on full data with fixed per-feature alphas ---
    ...
    coefs_scaled = dict(zip(ALL_FEATURE_KEYS, w_full.tolist()))
    log.info("[trainer] In-sample R²=%s  CV R²=%s  |  coefficients (scaled): %s",
             r_squared_insample, best_cv_r2, coefs_scaled)
    ...
    weights_row = {
        **coefs_scaled,
        ...
        "lambda":       0.0,  # per-feature alphas, not a single λ
    ...
    log.info(
        "[trainer] ✓ Saved model_id=%s  R²=%s  CV_R²=%s  n=%s  per-feature α=%s",
        model_id, r_squared_insample, best_cv_r2, len(filtered), list(PER_FEATURE_ALPHAS),
    )
    return result